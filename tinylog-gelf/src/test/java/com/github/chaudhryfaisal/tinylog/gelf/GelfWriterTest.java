...
public class GelfWriterTest {
    ...
    public void getRequiredLogEntryValuesIncludesAdditionalFields() {
        final Writer gelfWriter = new GelfWriter("localhost", 12201, GelfTransports.UDP, "localhost",
                EnumSet.of(LogEntryValue.EXCEPTION, LogEntryValue.THREAD), Collections.<String, Object>emptyMap(),
                512, 1000, 500, -1, false);

        assertThat(gelfWriter.getRequiredLogEntryValues(), hasItems(
                LogEntryValue.DATE, LogEntryValue.LEVEL, LogEntryValue.MESSAGE,
                LogEntryValue.EXCEPTION, LogEntryValue.THREAD));
    }
    ...
    public void testWrite() throws Exception {
        ...
        final GelfWriter gelfWriter = new GelfWriter("localhost", 12201, GelfTransports.UDP, "myHostName",
                EnumSet.of(LogEntryValue.EXCEPTION, LogEntryValue.THREAD),
                Collections.<String, Object>singletonMap("staticField", "TEST"), 512, 1000, 500, -1, false);

        @SuppressWarnings("all") final RuntimeException exception = new RuntimeException("BOOM!");
        exception.fillInStackTrace();
        final Timestamp now = new LegacyTimestamp();
        ...
        final LogEntry logEntry = new LogEntry(now, thread, context,
                "TEST-ClassName", "TEST-MethodName", "TEST-FileName", 42,
                "Test 123", Level.INFO, "Test 123", exception);
        ...
        assertThat(new Date((long) (message.getTimestamp() * 1000L)), equalTo(now.toDate()));
...
//        Method m = Configuration.class.getDeclaredMethod("load");
//        m.setAccessible(true); //This is a key statement for accessing private methods
//        Properties properties = (Properties) m.invoke(null);
